...
class ToraStockGateway(BaseGateway):
    ...
    def send_order(self, req: OrderRequest) -> str:
        """"""
        orderid=self.td_api.send_order(req)
        return orderid
...
class ToraTdApi(traderapi.CTORATstpTraderSpi):
    ...
    def OnErrRtnOrderInsert(self, data: dict, error: dict) -> None:
        ...
        self.gateway.write_error(f"拒单({order_id}):", error)
    ...
    def send_order(self, req: OrderRequest):
        ...
        info.ShareholderID = self.shareholder_ids.get(req.exchange,"")
        ...
        #sam
        order_id = f"{self.frontid}_{self.sessionid}_{order_ref}"
        # order_id = f"{self.gateway_name}.{self.frontid}_{self.sessionid}_{order_ref}"
